...
module chamfered_text(legend_text,
  ...
  cylinder_width = legend_depth*chamfer_slope;
  
  translate([0,0,legend_depth])
  mirror([0,0,1])
  difference(){
        ...
        translate([0,0,-legend_depth]) // Carve out text off cube
          linear_extrude(legend_depth*2)
            text( text=legend_text, 
                  ...
                  size=legend_size,
                  valign="center",
                  halign="center");  // Carve out text off cube
      }

      translate([0,0,chamfer_depth/2]) // Carve chamfer off text using pointy cylinder 
